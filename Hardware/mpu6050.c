...
{ 
	...
	MPU_Set_Rate(10);						//设置采样率50Hz
	MPU_Write_Byte(MPU_INT_EN_REG,0X00);	//关闭所有中断（关的是MPU6050自身产生中断（作为主机时））
	MPU_Write_Byte(MPU_USER_CTRL_REG,0X00);	//I2C主模式关闭
	MPU_Write_Byte(MPU_FIFO_EN_REG,0X00);	//关闭FIFO
	MPU_Write_Byte(MPU_INTBP_CFG_REG,0X80);	//INT引脚低电平有效（可以直接接单片机进行外部中断下降沿检测）
...
void DMP_Init(void) {
	...
	delay_ms(200);          
	OLED_Clear();
	OLED_ShowString(1,5,"init ok!");						 //显示字符串
	delay_ms(800);
    MPU_FLAG = 1;
}
